...
CREATE TABLE userGroupamania (id INT(11) AUTO_INCREMENT PRIMARY KEY, InputPseudo varchar(100) UNIQUE KEY,InputName varchar(100), InputLastName varchar(100), InputEmail varchar(255) UNIQUE KEY, InputPassword varchar(255), isAdmin tinyint(1) );
...
CREATE TABLE CommentGroupomania (Id_commentaire INT(11) AUTO_INCREMENT PRIMARY KEY, id_PostComment int(11) , date_commentaire timestamp default CURRENT_TIMESTAMP, commentaire varchar(500), createur int(11), FOREIGN Key (id_PostComment ) REFERENCES PostGroupomania (id_Post) ON DELETE CASCADE );
